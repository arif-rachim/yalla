...
var yalla = (function () {
                        ...
                        if (xmlhttp.readyState == 4) {
                            ...
                            if (dependency.length > 0) {
                                Promise.all(dependency.map(function (dependency) {
                                    ...
                                    resolve(executeScript(xmlhttp.responseText));
                                }).catch(function (err) {
                                    console.log(err);
                                });
                            } else {
    ...
    yalla.idom.notifications.nodesCreated = function (nodes) {
        yalla.dispatchEvent('nodesCreated', nodes);
    };

    yalla.idom.notifications.nodesDeleted = function (nodes) {
        yalla.dispatchEvent('nodesDeleted', nodes);
    };
    ...
    /// Finally HAVANA have its own redux system, re-engineer redux api ! sweet !!!!
