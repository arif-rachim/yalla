...
var HtmlTemplateCollection = function () {
    ...
    _createClass(HtmlTemplateCollection, [{
        ...
        value: function _init() {
            var newTemplateCollection = this;
            var index = newTemplateCollection.items.length;
            while (index--) {
                var item = newTemplateCollection.items[index];
                var key = newTemplateCollection.keyFn.apply(newTemplateCollection, [item]);
                var htmlTemplate = newTemplateCollection.templateFn.apply(newTemplateCollection, [item, index, newTemplateCollection.items]);
...
var HtmlTemplate = function () {
    ...
    _createClass(HtmlTemplate, [{
        ...
        value: function _lookDynamicNodesFromPath() {
            ...
            var dynamicNodes = this.dynamicNodesPath.map(function (path) {
                    ...
                    Array.from(node.attributes).reduce(function (results, attribute) {
                        if (attribute.nodeValue.indexOf(PLACEHOLDER) >= 0) {
                            ...
                            for (var i = 0; i < dynamicLength; i++) {
                                ...
                                var path = [{ name: attribute.nodeName, dynamicLength: dynamicLength }].concat(getPath(attribute.ownerElement));
