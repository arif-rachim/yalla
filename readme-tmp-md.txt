...
**hello world**
---
Untuk merender hello world kita dapat menulis seperti berikut :
```javascript
render(`Hello World`,document.body);
```

Kode diatas maksudnya kita akan merender String hello World kedalam body tag.

**`render` : Fungsi yang akan merender parameter pertama kedalam parameter kedua**
---
`render` adalah sebuah function yang menerima 2 parameter, parameter pertama adalah
object yang akan dirender dan parameter kedua adalah container dimana object akan dirender.

Parameter pertama dari `render` dapat berupa `string`, `boolean`, `date`, `number`, `Promise`, `HtmlTemplate` dan `HtmlTemplateCollection`.
Parameter kedua merupakan DOM node, kita bisa menggunakan `document.body` atau `document.getElementById` untuk parameter kedua.

Untuk merender html kita bisa berikan ke parameter pertama `HtmlTemplate` object dengan menggunakan
tag `html`. Seperti berikut :

```javascript
render(html`<button>Hello World</button>`,document.body);
```

Kode diatas artinya kita ingin me render tombol Hello World ke document.body element.

**`html` : Template Tag untuk merender html template**
---
`html` tag dibelakang layar adalah sebuah ES6 Template Tag.`html` menghasilkan HtmlTemplate object,
 yang berisikan informasi mengenai static strings, dan dynamic value.
 `html` tag dapat diambil dari object `yalla.Context`.

> `yalla.Context` adalah object yang menyimpan cache dari `html` dan `htmlCollection`
Tag. Untuk kasus hybrid application dimana kita bisa memiliki multiple sub-applikasi (bukan single page app),
kita bisa pisahkan context dari sub-aplikasi dengan memberikan alias dari `html` dan `htmlCollection` dari setiap `Context`.
...
Rendering `html dalam html` :
```javascript
render(html`<div>This is Div ${html`<div>This is Sub-Div</div>`} </div>,document.body);
```

Rendering dengan expression :
```javascript
let displayMe = false;
render(html`<div>This is Div ${displayMe ? html`<div>This is Sub-Div</div>` : ''} </div>,document.body);
```

Kita juga dapat mendengarkan DOM event dengan menset nilai dari `oneventname` dengan expression `e => {}`

**Events**
---

Event di HtmlTemplate dapat di panggil dengan menggunakan callback expression `e => {}`.
Berikut ini adalah contoh kita ingin mendengarkan event `onclick` dari sebuah `button`.
...
Kita juga dapat mempassing parameter ke dalam listener kita.
```javascript
...
Selain Event, HtmlTemplate juga dapat menset nilai dari attribute dengan menggunakan Template Literal.


**Attribute**
---

Attribute di HtmlTemplate dapat di set nilainya dengan menggunakan TemplateLiteral tag, berikut ini adalah
cara untuk men-set nilai dari attribute warna dan color.
